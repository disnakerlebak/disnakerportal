...
<x-dropdown :id="'admin-actions-'.$admin->id">
    <x-dropdown-item
        ...
        onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'edit-admin-{{ $admin->id }}' }))">
        Edit
    </x-dropdown-item>

    <x-dropdown-item
        class="{{ $admin->status === 'active' ? 'text-orange-300 hover:text-orange-100' : 'text-green-300 hover:text-green-100' }}"
        onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'confirm-toggle-{{ $admin->id }}' }))">
        {{ $admin->status === 'active' ? 'Nonaktifkan' : 'Aktifkan' }}
    </x-dropdown-item>

    <x-dropdown-item
        class="text-red-300 hover:text-red-100"
        onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'confirm-delete-{{ $admin->id }}' }))">
        Hapus
    </x-dropdown-item>
</x-dropdown>
  ...
  <x-modal id="confirm-delete-{{ $admin->id }}" size="md" title="Konfirmasi Hapus">
      <div class="px-6 py-5">
          ...
          <div class="flex justify-end gap-2 pt-3">
              <button data-close-modal="confirm-delete-{{ $admin->id }}" class="px-4 py-2 rounded border border-slate-700 bg-slate-800 text-slate-200 hover:bg-slate-700">Batal</button>
  ...
  <x-modal id="confirm-toggle-{{ $admin->id }}" size="md" title="Konfirmasi">
      <div class="px-6 py-5">
          ...
          <div class="flex justify-end gap-2 pt-3">
              <button data-close-modal="confirm-toggle-{{ $admin->id }}" class="px-4 py-2 rounded border border-slate-700 bg-slate-800 text-slate-200 hover:bg-slate-700">Batal</button>
