  ...
  <div class="max-w-6xl mx-auto px-6 space-y-8 text-slate-100">
    ...
    <div class="grid md:grid-cols-3 sm:grid-cols-2 gap-4">
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 shadow text-center">
        ...
        <div class="text-3xl font-bold text-indigo-400 mt-1"><?php echo e($pending); ?></div>
      </div>
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 shadow text-center">
        <p class="text-slate-400 text-xs">AK1 Aktif</p>
        <div class="text-3xl font-bold text-emerald-400 mt-1"><?php echo e($approvedActive); ?></div>
      </div>
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 shadow text-center">
        <p class="text-slate-400 text-xs">Ditolak Bulan Ini</p>
        <div class="text-3xl font-bold text-rose-400 mt-1"><?php echo e($rejectedThisMonth); ?></div>
      </div>
    </div>
    ...
    <div class="grid md:grid-cols-3 gap-4">
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
        ...
        <div class="text-3xl font-bold"><?php echo e($totalPencaker); ?></div>
        <div class="mt-3 grid grid-cols-2 gap-2 text-sm">
          <div class="rounded border border-slate-800 p-3">
            <div class="text-slate-400">Lengkap Profil</div>
            <div class="font-semibold text-emerald-400"><?php echo e($lengkapProfil); ?></div>
          </div>
          <div class="rounded border border-slate-800 p-3">
            <div class="text-slate-400">Belum Lengkap</div>
            <div class="font-semibold text-yellow-400"><?php echo e($belumLengkap); ?></div>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
        ...
        <div class="text-3xl font-bold text-indigo-300"><?php echo e($activeSeekers); ?></div>
        <?php
        ...
        <div class="mt-3 space-y-2 text-sm">
          <div class="flex items-center justify-between"><span class="text-slate-400">Dengan Pelatihan</span><span class="font-medium"><?php echo e($withTraining); ?> (<?php echo e($pctTrain); ?>%)</span></div>
          ...
          <div class="flex items-center justify-between mt-2"><span class="text-slate-400">Dengan Pengalaman</span><span class="font-medium"><?php echo e($withWork); ?> (<?php echo e($pctWork); ?>%)</span></div>
      ...
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
        ...
        <div class="h-72 opacity-0 scale-90 transition duration-300 overflow-visible" wire:ignore
             x-data="chartComponent({type:'gender', initialData: <?php echo \Illuminate\Support\Js::from($genderChart)->toHtml() ?>})" x-init="init()">
    ...
    <div class="grid md:grid-cols-2 gap-4">
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
        ...
        <div class="h-80 opacity-0 scale-90 transition duration-300" wire:ignore
             x-data="chartComponent({type:'education', initialData: <?php echo \Illuminate\Support\Js::from($educationChart)->toHtml() ?>})" x-init="init()">
          <canvas x-ref="canvas"></canvas>
        </div>
      </div>
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
        ...
        <div class="h-96 opacity-0 scale-90 transition duration-300" wire:ignore
             x-data="chartComponent({type:'district', initialData: <?php echo \Illuminate\Support\Js::from($districtChart)->toHtml() ?>})" x-init="init()">
          <canvas x-ref="canvas"></canvas>
        </div>
      </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-4">
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
        ...
        <div class="h-80 opacity-0 scale-90 transition duration-300" wire:ignore
             x-data="chartComponent({type:'monthly', initialData: <?php echo \Illuminate\Support\Js::from(['labels'=>$monthlyLabels, 'data'=>$monthlyCounts])->toHtml() ?>})" x-init="init()">
          <canvas x-ref="canvas"></canvas>
        </div>
      </div>
      <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-5 shadow">
            ...
            <tbody class="divide-y divide-slate-800">
              <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $recentLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-slate-800/40">
                  <td class="p-2"><?php echo e($log['time'] ?? '-'); ?></td>
                  <td class="p-2"><?php echo e($log['actor'] ?? '-'); ?></td>
                  <td class="p-2"><?php echo e(ucfirst($log['action'] ?? '-')); ?></td>
                  <td class="p-2"><?php echo e($log['to'] ?? '-'); ?></td>
                </tr>
    ...
    <p class="text-xs text-slate-400">Data diperbarui: <?php echo e($lastUpdated); ?> WIB</p>
  </div>

  <?php if (! $__env->hasRenderedOnce('4bf3e548-779f-4718-8c6b-33720857aec5')): $__env->markAsRenderedOnce('4bf3e548-779f-4718-8c6b-33720857aec5'); ?>
...
<?php /**PATH C:\xampp\htdocs\disnakerportal\resources\views/livewire/admin/dashboard-stats.blade.php ENDPATH**/ ?>